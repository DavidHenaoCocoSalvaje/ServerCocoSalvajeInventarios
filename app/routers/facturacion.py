...
async def facturar_compra_invoice(invoice: Invoice, session: AsyncSession = Depends(get_async_session)) -> bool:
    ...
    try:
        ...
        if wo_tercero and wo_tercero.ciudad.id:
            ...
            if not invoice.emisor.ciudad_id:
                raise Exception(
                    f'El emisor {invoice.emisor.razonsocial} no proporciona código de ciudad en la factura electrónica {invoice.id}.'
                )
